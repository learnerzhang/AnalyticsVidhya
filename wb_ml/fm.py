...
class FmModel(object):
	def __init__(self, i, x, y, feature_num, valid_num, hidden_num):
		...
		print(expressings, expressings_reduce)

		x_tmp = tf.tile(x_, [1, 1, self.hidden_num])  # SAMPLE_NUM * VALID_NUM * HIDDEN_NUM
		w_tmp = tf.nn.embedding_lookup(self.weight_mix, self.i)  # SAMPLE_NUM * VALID_NUM * HIDDEN_NUM

		embeddings = tf.multiply(x_tmp, w_tmp)  # SAMPLE_NUM * VALID_NUM * HIDDEN_NUM
...
with tf.Session() as sess:
	...
	for epoch in range(EPOCH):
		_training_op, _loss = sess.run([training_op, fm.loss], feed_dict={i: np.array([np.random.choice(20, 8) for cnt in range(10)]), x: np.random.rand(10, 8), y: np.random.randint(2, size=10)})
		_accuracy = sess.run([accuracy], feed_dict={i: np.array([np.random.choice(20, 8) for cnt in range(5)]), x: np.random.rand(5, 8), y: np.random.randint(2, size=5)})
